...
let rec readk : type s a. Buffy.R.source -> (s, a) Descr.t -> a Buffy.R.readed =
 fun source encoding ->
    ...
    (match Buffy.R.peak_stop source with
     ...
     | Some expected_stop ->
       let rec fold (source : Buffy.R.source) reversed_list =
         assert (source.readed <= expected_stop);
  ...
  | Headered { mkheader = _; headerencoding; mkencoding; equal = _; maximum_size = _ } ->
    ...
    (match mkencoding header with
     | Error msg ->
       ...
       Failed { source; error }
     | Ok (EDynamic encoding) -> readk source encoding
     | Ok (EStatic encoding) -> readk source encoding)
  ...
  | TupNil -> Buffy.R.Readed { source; value = [] }
  | TupCons (tupler, t, ts) ->
    (match tupler with
...
let readk : type s a. Buffy.R.source -> (s, a) Descr.t -> a Buffy.R.readed =
 fun source encoding ->
  if source.offset < 0
...
let%expect_test _ =
  ...
  w
    ...
    (fun fmt [ (); l; r; () ] -> Format.fprintf fmt "%lx_%lx" l r);
  [%expect {|
    Error: new source blob is too small to continue |}];
  w
